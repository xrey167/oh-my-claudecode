...
function resolveSupportedShellAffinity(shellPath) {
  ...
  const name = (0, import_path6.basename)(shellPath.replace(/\\/g, "/")).replace(/\.(exe|cmd|bat)$/i, "");
...
function shellNameFromPath(shellPath) {
  const shellName = (0, import_path6.basename)(shellPath.replace(/\\/g, "/"));
...
function isAbsoluteLaunchBinaryPath(value) {
  return (0, import_path6.isAbsolute)(value) || import_path6.win32.isAbsolute(value);
}
...
async function killWorkerPanes(opts) {
  ...
  const shutdownPath = (0, import_path6.join)(cwd, ".omc", "state", "team", teamName, "shutdown.json");
  try {
...
var import_child_process2, import_fs5, import_path6, import_util, import_promises, sleep, TMUX_SESSION_PREFIX, promisifiedExec, promisifiedExecFile, SUPPORTED_POSIX_SHELLS, ZSH_CANDIDATES, BASH_CANDIDATES, DANGEROUS_LAUNCH_BINARY_CHARS;
...
function loadSecurityFromConfigFiles() {
  const paths = [
    ...
    (0, import_path4.join)(getConfigDir2(), "claude-omc", "config.jsonc")
  ];
...
function getTrustedPrefixes() {
  ...
  const custom = (process.env.OMC_TRUSTED_CLI_DIRS ?? "").split(":").map((part) => part.trim()).filter(Boolean).filter((part) => (0, import_path5.isAbsolute)(part));
  trusted.push(...custom);
  return trusted;
}
function isTrustedPrefix(resolvedPath) {
  const normalized = (0, import_path5.normalize)(resolvedPath);
  return getTrustedPrefixes().some((prefix) => normalized.startsWith((0, import_path5.normalize)(prefix)));
}
...
function resolveCliBinaryPath(binary) {
  ...
  const resolvedPath = (0, import_path5.normalize)(firstLine);
...
function validateBinaryRef(binary) {
  if ((0, import_path5.isAbsolute)(binary)) return;
...
function resolveBinaryPath(binary) {
  ...
  if ((0, import_path5.isAbsolute)(binary)) return binary;
  try {
    ...
    const isResolvedAbsolute = !!firstPath && ((0, import_path5.isAbsolute)(firstPath) || import_path5.win32.isAbsolute(firstPath));
...
function getPackageDir2() {
  if (typeof __dirname !== "undefined" && __dirname) {
    const currentDirName = (0, import_path7.basename)(__dirname);
    const parentDirName = (0, import_path7.basename)((0, import_path7.dirname)(__dirname));
    if (currentDirName === "bridge") {
      return (0, import_path7.join)(__dirname, "..");
    }
    if (currentDirName === "agents" && (parentDirName === "src" || parentDirName === "dist")) {
      return (0, import_path7.join)(__dirname, "..", "..");
    }
...
function getValidAgentRoles() {
  ...
  try {
    const agentsDir = (0, import_path7.join)(getPackageDir2(), "agents");
    const files = (0, import_fs6.readdirSync)(agentsDir);
    _cachedRoles = files.filter((f) => f.endsWith(".md")).map((f) => (0, import_path7.basename)(f, ".md")).sort();
  } catch (err) {
...
function buildInstructionPath(...parts) {
  return (0, import_path8.join)(...parts).replaceAll("\\", "/");
}
...
async function composeInitialInbox(teamName, workerName2, content, cwd) {
  const inboxPath = (0, import_path8.join)(cwd, `.omc/state/team/${teamName}/workers/${workerName2}/inbox.md`);
  await (0, import_promises2.mkdir)((0, import_path8.dirname)(inboxPath), { recursive: true });
  await (0, import_promises2.writeFile)(inboxPath, content, "utf-8");
}
async function ensureWorkerStateDir(teamName, workerName2, cwd) {
  const workerDir = (0, import_path8.join)(cwd, `.omc/state/team/${teamName}/workers/${workerName2}`);
  await (0, import_promises2.mkdir)(workerDir, { recursive: true });
  const mailboxDir = (0, import_path8.join)(cwd, `.omc/state/team/${teamName}/mailbox`);
  await (0, import_promises2.mkdir)(mailboxDir, { recursive: true });
  const tasksDir = (0, import_path8.join)(cwd, `.omc/state/team/${teamName}/tasks`);
...
async function writeWorkerOverlay(params) {
  ...
  const overlayPath = (0, import_path8.join)(cwd, `.omc/state/team/${teamName}/workers/${workerName2}/AGENTS.md`);
  await (0, import_promises2.mkdir)((0, import_path8.dirname)(overlayPath), { recursive: true });
...
function safeRealpath(p) {
  try {
    ...
    const parent = (0, import_path9.dirname)(p);
    const name = (0, import_path9.basename)(p);
    try {
      return (0, import_path9.resolve)((0, import_fs7.realpathSync)(parent), name);
    } catch {
      return (0, import_path9.resolve)(p);
    }
  }
}
function validateResolvedPath(resolvedPath, expectedBase) {
  ...
  const rel = (0, import_path9.relative)(absBase, absResolved);
  if (rel.startsWith("..") || (0, import_path9.resolve)(absBase, rel) !== absResolved) {
...
function absPath(cwd, relativePath) {
  return (0, import_path10.isAbsolute)(relativePath) ? relativePath : (0, import_path10.join)(cwd, relativePath);
}
function teamStateRoot(cwd, teamName) {
  return (0, import_path10.join)(cwd, TeamPaths.root(teamName));
}
function getTaskStoragePath(cwd, teamName, taskId) {
  if (taskId !== void 0) {
    return (0, import_path10.join)(cwd, TeamPaths.taskFile(teamName, taskId));
  }
  return (0, import_path10.join)(cwd, TeamPaths.tasks(teamName));
}
...
function acquireTaskLock(teamName, taskId, opts) {
  ...
  ensureDirWithMode(dir);
  const lockPath = (0, import_path11.join)(dir, `${sanitizeTaskId(taskId)}.lock`);
...
function canonicalTasksDir(teamName, cwd) {
  ...
  validateResolvedPath(dir, (0, import_path11.join)(root, ".omc", "state", "team"));
  return dir;
}
function failureSidecarPath(teamName, taskId, cwd) {
  return (0, import_path11.join)(canonicalTasksDir(teamName, cwd), `${sanitizeTaskId(taskId)}.failure.json`);
}
...
function stateRoot(cwd, teamName) {
  validateTeamName(teamName);
  return (0, import_path12.join)(cwd, `.omc/state/team/${teamName}`);
}
async function writeJson(filePath, data) {
  await (0, import_promises3.mkdir)((0, import_path12.join)(filePath, ".."), { recursive: true });
...
function taskPath(root, taskId) {
  return (0, import_path12.join)(root, "tasks", `${taskId}.json`);
}
...
async function startTeam(config) {
  ...
  await (0, import_promises3.mkdir)((0, import_path12.join)(root, "tasks"), { recursive: true });
  await (0, import_promises3.mkdir)((0, import_path12.join)(root, "mailbox"), { recursive: true });
  await writeJson((0, import_path12.join)(root, "config.json"), config);
  for (let i = 0; i < tasks.length; i++) {
    const taskId = String(i + 1);
    await writeJson((0, import_path12.join)(root, "tasks", `${taskId}.json`), {
      id: taskId,
  ...
  await writeJson((0, import_path12.join)(root, "config.json"), runtime.config);
...
async function monitorTeam(teamName, cwd, workerPaneIds) {
  ...
  try {
    ...
    const taskFiles = await readdir2((0, import_path12.join)(root, "tasks"));
    for (const f of taskFiles.filter((f2) => f2.endsWith(".json"))) {
      const task = await readJsonSafe((0, import_path12.join)(root, "tasks", f));
  ...
  for (let i = 0; i < workerPaneIds.length; i++) {
    ...
    const heartbeatPath = (0, import_path12.join)(root, "workers", wName, "heartbeat.json");
    ...
    try {
      ...
      const [doneSignals, aliveResults] = await Promise.all([
        Promise.all(workers.map(([wName]) => {
          const donePath = (0, import_path12.join)(root, "workers", wName, "done.json");
      ...
      if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
        ...
        try {
          const root = stateRoot(runtime.cwd, runtime.teamName);
          await writeJson((0, import_path12.join)(root, "watchdog-failed.json"), {
...
async function shutdownTeam(teamName, sessionName2, cwd, timeoutMs = 3e4, workerPaneIds, leaderPaneId, ownsWindow) {
  const root = stateRoot(cwd, teamName);
  await writeJson((0, import_path12.join)(root, "shutdown.json"), {
    requestedAt: (/* @__PURE__ */ new Date()).toISOString(),
    teamName
  });
  const configData = await readJsonSafe((0, import_path12.join)(root, "config.json"));
  ...
  if (!isCliWorkerTeam) {
    ...
    while (Date.now() < deadline && expectedAcks.length > 0) {
      for (const wName of [...expectedAcks]) {
        const ackPath = (0, import_path12.join)(root, "workers", wName, "shutdown-ack.json");
...
async function appendTeamEvent(teamName, event, cwd) {
  ...
  await (0, import_promises4.mkdir)((0, import_path13.dirname)(p), { recursive: true });
...
function checkCwdParity(claimsCwd, runtimeCwd, mode, policy) {
  ...
  const claimsCwdCanonical = (0, import_path14.resolve)(claimsCwd);
  const runtimeCwdCanonical = (0, import_path14.resolve)(runtimeCwd);
...
async function writeAtomic(filePath, data) {
  ...
  await (0, import_promises5.mkdir)((0, import_path15.dirname)(filePath), { recursive: true });
  const tmpPath = `${filePath}.tmp.${process.pid}.${Date.now()}`;
...
async function withDispatchLock(teamName, cwd, fn) {
  ...
  const ownerPath = (0, import_path16.join)(lockDir, "owner");
  const ownerToken = `${process.pid}.${Date.now()}.${Math.random().toString(16).slice(2)}`;
  const timeoutMs = resolveDispatchLockTimeoutMs(process.env);
  const deadline = Date.now() + timeoutMs;
  let pollMs = DISPATCH_LOCK_INITIAL_POLL_MS;
  await (0, import_promises6.mkdir)((0, import_path16.dirname)(lockDir), { recursive: true });
  while (true) {
    try {
...
async function writeDispatchRequestsToFile(teamName, requests, cwd) {
  const path4 = absPath(cwd, TeamPaths.dispatchRequests(teamName));
  const dir = (0, import_path16.dirname)(path4);
  ensureDirWithMode(dir);
...
async function startTeamV2(config) {
  ...
  const leaderCwd = (0, import_path17.resolve)(config.cwd);
  ...
  await (0, import_promises7.mkdir)((0, import_path17.join)(leaderCwd, ".omc", "state", "team", sanitized, "mailbox"), { recursive: true });
  for (let i = 0; i < config.tasks.length; i++) {
    const taskId = String(i + 1);
    const taskFilePath = absPath(leaderCwd, TeamPaths.taskFile(sanitized, taskId));
    await (0, import_promises7.mkdir)((0, import_path17.join)(taskFilePath, ".."), { recursive: true });
...
async function checkWatchdogFailedMarker(stateRoot2, startTime) {
  const markerPath = (0, import_path18.join)(stateRoot2, "watchdog-failed.json");
  let raw;
  try {
...
function collectTaskResults(stateRoot2) {
  const tasksDir = (0, import_path18.join)(stateRoot2, "tasks");
  try {
    const files = (0, import_fs18.readdirSync)(tasksDir).filter((f) => f.endsWith(".json"));
    return files.map((f) => {
      try {
        const raw = (0, import_fs18.readFileSync)((0, import_path18.join)(tasksDir, f), "utf-8");
        const task = JSON.parse(raw);
        return {
...
`);
  ...
  const stateRoot2 = (0, import_path18.join)(cwd, `.omc/state/team/${teamName}`);
  const config = {
    teamName,
    workerCount,
    agentTypes,
